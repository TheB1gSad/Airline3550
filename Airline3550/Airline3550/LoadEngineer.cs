...
namespace Airline3550
{
	internal class LoadEngineer
	{
         ...
         */
		public bool setFlightRoute(string departureTime, int flightID, string departure, string arrival, string date)
		{
			...
			{
				new Flight { departureTime = departureTime, arrivalTime = arrivalTime.ToString() + ":00",departureDate=date, flightID = flightID, flightDistance = distance, departure = departure, arrival = arrival, planeType = planeType, price = price}
			};
			...
			addSeatsList(distance, flightID.ToString());
			return true;
		}

		private void addSeatsList(int distance, string flightID)
		{
			List<string> seatList = new List<string>();
			seatList.Add(flightID);
			if (distance > 600)
			{
				for (int i = 0; i < 189; i++)
				{
			...
			string filePath = Path.GetDirectoryName(Application.ExecutablePath);
			string filecsv = Path.Combine(filePath, "flightSeats.csv");
			var config = new CsvConfiguration(CultureInfo.InvariantCulture)
			{
				// Don't write the header again.
				HasHeaderRecord = false,
			};
			string records = string.Join(",", seatList.ToArray());

			File.AppendAllText(filecsv, "\n"+records);
